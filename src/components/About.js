import React from "react";
// count up
import CountUp from "react-countup";
// intersection
import { useInView } from "react-intersection-observer";
// motion
import { motion } from "framer-motion";
// variants
import { fadeIn } from "../variants";

const About = () => {
    const [ref, inView] = useInView({
        threshold: 0.5,
    });
    return (
        <secion className="section" id="about" ref={ref}>
            <div className="container mx-auto">
                <div className="flex flex-col gap-y-10 lg:flex-row lg:items-center lg:gap-x-20 lg:gap-y-0 lg:h-screen">
                    {/* image */}
                    <motion.div
                        variants={fadeIn("right", 0.5)}
                        initial="hidden"
                        whileInView={"show"}
                        viewport={{ once: false, amount: 0.3 }}
                        className="flex-1 bg-about bg-contain bg-no-repeat h-[640px] mix-blend-lighten bg-top"
                    ></motion.div>
                    {/* text */}
                    <motion.div
                        variants={fadeIn("left", 0.5)}
                        initial="hidden"
                        whileInView={"show"}
                        viewport={{ once: false, amount: 0.3 }}
                        className="flex-1 "
                    >
                        <h2 className="h2 text-accent">About me.</h2>
                        <h3 className="h3 mb-4">
                            I am a Full Stack Web Developer with a passion
                            forhjashdjkhasjkdhkashdjkasdasdasda
                        </h3>
                        <p className="mb-6">
                            djaskldhjakshjdkljasjdjkjhaskjldj aklsjdkl ajs
                            dkljaskldj klasjd klajskld jaklsdj
                            klasjdklajskldjkalsjdklajs kldjakls
                        </p>
                        {/* stats */}
                        <div className="flex gap-x-6 lg:gap-x-10 mb-12 ">
                            <div>
                                <div className="text-[40px] font-tertiary text-gradient mb-2">
                                    {inView ? (
                                        <CountUp
                                            start={0}
                                            end={13}
                                            duration={3}
                                        />
                                    ) : null}
                                </div>
                                <div className="font-primary text-sm traking-[2px]">
                                    Years of <br /> Experience
                                </div>
                            </div>
                            <div>
                                <div className="text-[40px] font-tertiary text-gradient mb-2">
                                    {inView ? (
                                        <CountUp
                                            start={0}
                                            end={13}
                                            duration={3}
                                        />
                                    ) : null}
                                </div>
                                <div className="font-primary text-sm traking-[2px]">
                                    Projects <br /> Completed
                                </div>
                            </div>
                            <div>
                                <div className="text-[40px] font-tertiary text-gradient mb-2">
                                    {inView ? (
                                        <CountUp
                                            start={0}
                                            end={13}
                                            duration={3}
                                        />
                                    ) : null}
                                </div>
                                <div className="font-primary text-sm traking-[2px]">
                                    Satisfied <br /> Clients
                                </div>
                            </div>
                        </div>
                        {/* button */}
                        <div className="flex gap-x-8 items-center">
                            <button className="btn btn-lg">Contact me</button>
                            <a href="#" className="text-gradient btn-link">
                                MY Portfolio
                            </a>
                        </div>
                    </motion.div>
                </div>
            </div>
        </secion>
    );
};

export default About;
